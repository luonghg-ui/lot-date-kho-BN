...
// Dynamic Proxy URL resolving
function getActiveProxyUrls() {
    const base = typeof getProxyUrl === 'function' ? getProxyUrl() : 'http://localhost:8081';
    return {
        base,
        wms: base + '/wms',
        move: base + '/wms/move',
        image: base + '/image',
        detail: base + '/wms/detail',
        updateToken: base + '/update-token'
    };
}
...
function getProxiedImageUrl(url) {
    ...
    if (targetUrl.startsWith('http')) {
        return `${getActiveProxyUrls().image}?url=${encodeURIComponent(targetUrl)}`;
    }
    return targetUrl;
}
...
async function fetchWmsData() {
    try {
        ...
        const response = await fetch(getActiveProxyUrls().wms);
...
async function fetchSheetData() {
    try {
        ...
        try {
            ...
            response = await fetch(`${getActiveProxyUrls().base}/csv?t=${Date.now()}`, { cache: 'no-store' });
        ...
        suggestions.innerHTML = filtered.map(p => `
            <div class="suggestion-item" onclick="window.selectProduct('${p.sku}')">
                ...
                <div class="flex-grow min-w-0">
                    <div class="font-bold text-theme-primary text-sm truncate">${p.name}</div>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="text-[10px] font-mono text-theme-muted">${p.sku}</span>
...
window.selectProduct = async (sku) => {
    ...
    document.getElementById('locationTableBody').innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-theme-muted">Đang tải...</td></tr>';
    document.getElementById('lotTableBody').innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-theme-muted">Đang tải...</td></tr>';

    // Fetch live system quantity from Detail API
    try {
        console.log(`Fetching live data for SKU: ${sku}...`);
        const response = await fetch(`${getActiveProxyUrls().detail}?sku=${sku}`);
        if (response.ok) {
            const result = await response.json();
            console.log("Full Detail API Response:", result);
            // If proxy scraped HTML, it returns skuData instead of data
            const detail = result.data || result.skuData;
            if (detail) {
                ...
                selectedProduct.availableQty = availableQty;
                selectedProduct.onHoldQty = onHoldQty;
                ...
                if (locations.length > 0) {
                    ...
                    locBody.innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-theme-muted italic">Không có vị trí kệ</td></tr>';
                }
                ...
                if (lots.length > 0) {
                    ...
                    lotBody.innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-theme-muted italic">Không có dữ liệu Lot</td></tr>';
                }

                // Fetch Move data
                try {
                    const moveResponse = await fetch(`${getActiveProxyUrls().move}?sku=${sku}`);
                    if (moveResponse.ok) {
...
function handleAddInventory(e, finishSKU = true) {
    ...
    const newItem = {
        ...
        onHoldQty: selectedProduct.onHoldQty || 0,
        lot,
        date,
        qty,
        note,
...
function renderInventory() {
    ...
    groups.forEach((items, sku) => {
        ...
        items.forEach((item, index) => {
            ...
            html += `
                <tr class="group transition-all bg-theme-hover">
                    ${index === 0 ? `
                        <td class="py-6 px-4 product-group-cell align-top" rowspan="${rowSpan}" style="width: 38%;">
                            <div class="sticky">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 class="text-theme-primary font-bold text-sm leading-tight">${item.name}</h4>
                                </div>
                                <div class="flex items-center gap-2 mb-3">
                                    <span class="text-[10px] font-mono text-theme-muted uppercase">${item.sku}</span>
                    ...
                    <td class="py-5 border-b border-slate-50">
                        <span class="text-theme-secondary text-xs font-medium">${formatDate(item.date)}</span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="text-[10px] font-bold text-theme-muted bg-theme-card px-2.5 py-1.5 rounded border border-theme">
                            ${item.location || '-'}
                        </span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="text-sm font-bold text-emerald-600">${(item.availableQty || 0).toLocaleString('vi-VN')}</span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="text-sm font-bold text-amber-600">${(item.onHoldQty || 0).toLocaleString('vi-VN')}</span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="text-sm font-semibold text-slate-500 dark:text-slate-400">${(item.systemQty || 0).toLocaleString('vi-VN')}</span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="text-lg font-bold text-theme-primary">${item.qty.toLocaleString('vi-VN')}</span>
                    </td>
                    <td class="py-5 text-center border-b border-slate-50">
                        <span class="badge ${badgeClass} shadow-sm inline-block min-w-[3.5rem] py-2">${diffText}</span>
                    </td>
                    <td class="py-5 text-right pr-4 border-b border-slate-50">
                        <button onclick="window.deleteItem(${item.id})" class="p-2 text-theme-muted hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
...
function copyToClipboard() {
    ...
    const headers = "Ngày\tSKU\tTên SP\tLot\tDate\tMã Vị Trí\tCó Sẵn\tĐang Giữ\tSL Thực\tSL Hệ Thống\tGhi Chú\n";
    const rows = inventory.map(item => 
        `${item.timestamp}\t${item.sku}\t${item.name}\t${item.lot}\t${item.date}\t${item.location || ''}\t${item.availableQty || 0}\t${item.onHoldQty || 0}\t${item.qty}\t${item.systemQty}\t${item.note || ''}`
    ).join('\n');
...
function exportToCSV() {
    if (inventory.length === 0) return;
    const headers = "Ngay,SKU,Ten SP,Lot,Date,Ma Vi Tri,Co San,Dang Giu,SL Thuc,SL He Thong,Ghi Chu\n";
...
async function saveToSheet() {
    ...
    try {
        // Build rows theo đúng thứ tự cột trong Google Sheet:
        // SKU | Sản Phẩm | Nhóm | Phân loại | Số Lot | Hạn Dùng | Mã Vị Trí | Có sẵn | Đang giữ | Tồn hệ thống | SL Thực | Lệch | Timestamp
        const rows = inventory.map(item => {
            const pInfo = products.find(p => p.sku === item.sku) || {};
            const groupLabel = item.group || pInfo.group || '';
            const isMed = (item.classification || '').toLowerCase().includes('thuốc') ||
                          (pInfo.classification || '').toLowerCase().includes('thuốc') ||
                          item.name.toLowerCase().includes('thuốc');
            const loaiSP = isMed ? 'Thuốc' : 'Không phải thuốc';
            const diff = item.qty - item.systemQty;
            return [
                item.sku,                          // A: SKU
                item.name,                         // B: Sản Phẩm
                groupLabel,                        // C: Nhóm
                loaiSP,                            // D: Phân loại
                item.lot,                          // E: Số Lot
                formatDate(item.date),             // F: Hạn Dùng
                item.location || '',               // G: Mã Vị Trí
                item.availableQty || 0,            // H: Có sẵn
                item.onHoldQty || 0,               // I: Đang giữ
                item.systemQty || 0,               // J: Tồn hệ thống
                item.qty,                          // K: SL Thực
                diff,                              // L: Lệch
        ...
        const payload = {
            rows: rows,
            // Giữ lại items để Apps Script cũ vẫn hoạt động (backward compat)
            items: inventory.map(item => ({
                ...
                onHoldQty: item.onHoldQty || 0,
                systemQty: item.systemQty || 0,
                qty: item.qty,
                lech: item.qty - item.systemQty,
