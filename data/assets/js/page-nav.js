 ...
 * Dùng: chỉ cần <script src="...page-nav.js"></script> là xong
 */
(function () {
  // ── Danh sách trang ──────────────────────────────────────────────────────
  const PAGES = [
  ...
  function buildModal() {
    ...
    const isRoot = !window.location.pathname.includes('/data/html/');
    let linksHtml = '';
    ...
    PAGES.forEach(p => {
      ...
      const targetHref = p.isRootPage
        ? (isRoot ? p.filename : '../../' + p.filename)
        : (isRoot ? 'data/html/' + p.filename : p.filename);

      linksHtml += `
        ${sectionLabel}
        <a href="${targetHref}" onclick="closePageNavigator()"
