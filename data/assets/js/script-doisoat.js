...
function showShiftDetail(vendor, shiftType) {
    ...
    const rows = employees.map(item => {
        ...
        return `<tr>
            ...
            <td style="font-weight:700; color:var(--text);">${getVal(item, 'name') || '—'}</td>
            <td><span class="product-sku">${getVal(item, 'msnv') || '—'}</span></td>
            <td><span class="badge-shelf" style="background:var(--border); color:var(--text);">${getVal(item, 'shift') || '—'}</span></td>
            ${tIn}${tOut}
    ...
    reconcileDetailContent.innerHTML = `
        <div class="drill-header">
            ...
            <div class="drill-title-wrap">
                ...
                <div class="drill-sub">${shiftSub} &nbsp;·&nbsp; <b style="color:var(--text);">${employees.length}</b> nhân viên</div>
            </div>
        </div>
