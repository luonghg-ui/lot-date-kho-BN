  ...
  let html = `
    <div style="margin-top:24px; padding-top:24px; border-top:1px solid rgba(255,255,255,0.05);">
        <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:20px;">
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 14px; text-align:center; min-width:80px; flex:1;">
                <div style="font-size:18px; font-weight:800; color:#10B981">${Number(sd.availableQuantity||0).toLocaleString()}</div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Có sẵn</div>
            </div>
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 14px; text-align:center; min-width:80px; flex:1;">
                <div style="font-size:18px; font-weight:800; color:#F59E0B">${Number(sd.onHoldQuantity||0).toLocaleString()}</div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Đang giữ</div>
            </div>
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 14px; text-align:center; min-width:80px; flex:1;">
                <div style="font-size:18px; font-weight:800; color:var(--text)">${locCount}</div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Kệ có hàng</div>
            </div>
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 14px; text-align:center; min-width:80px; flex:1;">
                <div style="font-size:18px; font-weight:800; color:${clsColor}">${cls}</div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Phân loại</div>
            </div>
            <div style="background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 14px; text-align:center; min-width:80px; flex:1;">
                <div style="font-size:15px; font-weight:800; color:${typeColor}; margin-top:2px;">${typeLabel}</div>
                <div style="font-size:11px; color:var(--text-muted); margin-top:4px;">Loại SP</div>
            </div>
        </div>

        <div style="font-size:12px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:var(--text-muted); margin-bottom:12px; margin-top:24px; display:flex; align-items:center; gap:6px;">
        ...
        <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; overflow: hidden;">
            <table style="width:100%; min-width:auto; border-collapse:collapse; font-size:13px; font-family:var(--font-display);">
                <thead><tr style="background: var(--surface);">
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Kệ</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Tồn kho</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Có sẵn</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Giữ</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Trạng thái</th>
                </tr></thead>
                <tbody>${activeLocs.map(l => `<tr>
                    ...
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:var(--text); font-weight:600;">${(l.stockQuantity||0).toLocaleString()}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:#10B981; font-weight:600;">${(l.availableQuantity||0).toLocaleString()}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:#F59E0B; font-weight:600;">${(l.onHoldQuantity||0).toLocaleString()}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border);"><span style="font-size:11px; padding:2px 8px; border-radius:4px; background:rgba(37,99,235,0.15); border:1px solid rgba(37,99,235,0.3); color:var(--primary);">${l.status||'—'}</span></td>
                </tr>`).join('')}</tbody>
            </table>
        </div>`;
  } else {
    html += '<div style="color:var(--text-muted); font-size:13px; font-style:italic; padding-left:4px;">Không có kệ nào có hàng</div>';
  }

  html += `<div style="font-size:12px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:var(--text-muted); margin-bottom:12px; margin-top:28px; display:flex; align-items:center; gap:6px;"><i class='bx bx-calendar-event' style="font-size:16px;"></i> Lot / Hạn sử dụng (${activeLots.length} lot còn hàng)</div>`;
  ...
  if (activeLots.length) {
    html += `
        <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; overflow: hidden;">
            <table style="width:100%; min-width:auto; border-collapse:collapse; font-size:13px; font-family:var(--font-display);">
                <thead><tr style="background: var(--surface);">
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Lot</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">HSD</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Nhập</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Xuất</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Có sẵn</th>
                </tr></thead>
                <tbody>${activeLots.slice(0,20).map(l => `<tr>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:var(--text); font-weight:600;">${l.lot||'—'}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:${expColor(l.expiredTime)}; font-weight:500;">${l.expiredDate||'—'}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:var(--text-muted);">${(l.inQuantity||0).toLocaleString()}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:var(--text-muted);">${(l.outQuantity||0).toLocaleString()}</td>
                    <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:#10B981; font-weight:600;">${(l.availableQuantity||0).toLocaleString()}</td>
                </tr>`).join('')}</tbody>
            </table>
        </div>`;
  } else {
    html += '<div style="color:var(--text-muted); font-size:13px; font-style:italic; padding-left:4px;">Không có lot còn hàng</div>';
  }

  html += `<div style="font-size:12px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:var(--text-muted); margin-bottom:12px; margin-top:28px; display:flex; align-items:center; gap:6px;"><i class='bx bx-history' style="font-size:16px;"></i> Lịch sử gán vị trí (Gần đây)</div>`;
  ...
  if (histories.length) {
    html += `
        <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; overflow: hidden;">
            <table style="width:100%; min-width:auto; border-collapse:collapse; font-size:13px; font-family:var(--font-display);">
                <thead><tr style="background: var(--surface);">
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Thời gian</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">User / Hành động</th>
                    <th style="text-align:left; padding:10px 14px; color:var(--text-muted); border-bottom:1px solid var(--border); font-weight:600;">Vị trí</th>
                </tr></thead>
                <tbody>${histories.slice(0,10).map(h => {
                    ...
                    return `<tr>
                        <td style="padding:10px 14px; border-bottom:1px solid var(--border); white-space:nowrap; color:var(--text-muted); font-family:var(--font-mono); font-size:12px;">${formattedDate}</td>
                        <td style="padding:10px 14px; border-bottom:1px solid var(--border); color:var(--text); font-weight:500;">${user}<br><small style="color:var(--text-muted); font-weight:400; font-family:var(--font-sans); margin-top:2px; display:inline-block;">${actionName}</small></td>
                        <td style="padding:10px 14px; border-bottom:1px solid var(--border);"><span style="display:inline-block; background:rgba(16,185,129,0.12); border:1px solid rgba(16,185,129,0.35); color:#10B981; border-radius:6px; padding:3px 10px; font-family:monospace; font-weight:700;">${loc}</span></td>
                    </tr>`;
                }).join('')}</tbody>
            </table>
        </div>`;
  } else {
    html += '<div style="color:var(--text-muted); font-size:13px; font-style:italic; padding-left:4px;">Dữ liệu lịch sử không có sẵn.</div>';
  }
...
window.openCatModal = function(type) {
    ...
    if (type === 'not_out') {
        exportBtnHTML = `
            <div style="display:flex; gap:12px;">
                <button onclick="exportFoundDataToOUT()" onmouseover="this.style.background='rgba(99,102,241,0.2)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='rgba(99,102,241,0.1)'; this.style.transform='translateY(0)';" style="background:rgba(99,102,241,0.1); color:var(--primary); border:1px solid rgba(99,102,241,0.3); padding:10px 20px; border-radius:14px; font-family:var(--font-display); font-size:0.85rem; font-weight:700; display:flex; align-items:center; gap:8px; cursor:pointer; transition:all 0.3s; box-shadow: 0 4px 15px rgba(99,102,241,0.1);">
    ...
    catModalHeader.innerHTML = `
        <div style="position:relative; width:100%; display:flex; justify-content:center; align-items:center; flex-direction:column; gap:16px; margin-bottom: 8px;">
            ...
            <div style="text-align:center;">
                <div style="font-family:var(--font-display); font-size:1.4rem; font-weight:800; color:var(--text); text-transform:uppercase; letter-spacing:0.02em;">${title}</div>
                <div style="display:inline-flex; align-items:center; gap:16px; margin-top:12px; background:var(--surface); border:1px solid var(--border); border-radius:100px; padding:6px 20px; box-shadow: inset 0 2px 5px rgba(0,0,0,0.15);">
                    ...
                    <strong style="font-size:1.15rem; color:var(--text); font-family:var(--font-display); text-shadow: 0 2px 10px rgba(255,255,255,0.2);">${formatN(moneySum)} đ</strong>
                </div>
            </div>
    ...
    catModalBody.innerHTML = `
        <div class="table-wrap" style="background: var(--surface); border: 1px solid var(--border); box-shadow: inset 0 2px 10px rgba(0,0,0,0.2); margin-top: 5px;">
