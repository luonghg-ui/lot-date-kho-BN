...
const server = http.createServer(async (req, res) => {
    try {
            ...
            req.on('end', () => {
                try {
                    ...
                    if (data.sid || data.token) {
                        const extract = (raw) => {
                            if (!raw) return { s: null, t: null };
                            let s = null, t = null;
                            const sidMatch = raw.match(/SID=([^;]+)/);
                            if (sidMatch) s = sidMatch[1];
                        ...
                        if (data.token) {
                            const { s, t } = extract(data.token);
                            if (s) SID = s;
                            if (t) TOKEN = t;
                            if (!s && !t) TOKEN = data.token;
                        }
                        if (data.sid) {
                            const { s, t } = extract(data.sid);
                            if (s) SID = s;
                            if (t) TOKEN = t;
                            if (!s && !t) SID = data.sid;
                        }
        ...
        if (path === '/wms/mapping-location' && method === 'GET') {
            ...
            // WMS không trả JSON hợp lệ (token hết hạn / redirect login)
            res.writeHead(200);
            res.end(JSON.stringify({ ok: false, message: 'Token hết hạn hoặc không có quyền truy cập WMS. Vui lòng cập nhật Token mới.', status: result.status }));
            return;
        }
        ...
        if (path === '/wms/sku-detail' && method === 'GET') {
            ...
            try {
                ...
                res.writeHead(200);
                res.end(JSON.stringify({ ok: parsed.ok !== false, sku, ...parsed }));
            } catch (_) {
                res.writeHead(200);
                res.end(JSON.stringify({ ok: false, message: 'Token hết hạn hoặc không có quyền truy cập WMS. Vui lòng cập nhật Token mới.', status: result.status }));
            }
